...
mongoose.connect("mongodb://127.0.0.1:27017/T3Db");
	...
	User.findOne({description: "Never gonna give you up."}).lean().then((res) => {
		Post.create({title: "Mock Post", posterId: res._id, body: "Lorem ipsum...", tags: ["Test", "Tags"]});
		Post.findOne({posterId: res._id}).lean().then((res2) => {
			Comment.create({commenterId: res._id, postId: res2._id, body: "Hello world"});
			Comment.find({postId: res2._id}).lean().then((res3) => {
				console.log(res3);
			});
		});
	}).catch((err) => {
		console.error(err);
	}).finally((fres) => console.log (fres));
...
apiRouter.get("/api/posts/:id", isAuth, async (req, res) => {
  try {
    const { id } = req.params;
		console.log(id);
...
apiRouter.get("/post/:id/comments", async (req, res) => {
  try {
    ...
    const comments = await Comment.find({ postId: ObjectId(postId) }).lean();
